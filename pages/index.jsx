...
const REPO_URL = 'https://github.com/siliconjungle/sdf-shapeshift';
...
export default function SdfMorphDemo() {
  ...
  useEffect(() => {
    ...
    if (frame >= FRAMES) {
      setPlaying(false);
      setDir(d => !d);           // flip direction after morph finishes
      return;
    }
    ...
    <div className={styles.zoneWrap}>
      <div
        ...
        onDrop={e => {
          stop(e);
          setOver(false);
          e.dataTransfer.files[0] && pick(e.dataTransfer.files[0]);
        }}
      >
      ...
      {img && (
        <small className={styles.dim}>{img.width}×{img.height}</small>
      )}
    </div>
  );
}
